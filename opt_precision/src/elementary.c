...
double trig_large_arg_handler(double x, bool is_cos) {
    double abs_x = fabs(x);
    ...
    while (abs_x > TWO_PI) {
        ...
        if (abs_x <= PI_4)
            return is_cos ? cos(is_negative ? -abs_x : abs_x) : sen(is_negative ? -abs_x : abs_x);
    }
    ...
    if (PI_4 < x && x <= THREE_PI_4)
        return is_cos ? -sen(x - PI_2) : _cos(x - PI_2);
    else if (THREE_PI_4 < x && x <= FIVE_PI_4)
        return is_cos ? -_cos(x - M_PI) : -sen(x - M_PI);
    else if (FIVE_PI_4 < x && x <= SEVEN_PI_4)
        return is_cos ? sen(x - THREE_PI_2) : -_cos(x - THREE_PI_2);
    else if (SEVEN_PI_4 < x && x <= TWO_PI)
        return is_cos ? _cos(x - TWO_PI) : sen(x - TWO_PI);
    else
        return is_cos ? _cos(x) : sen(x);
}

double sen_large(double x) {
    return trig_large_arg_handler(x, false);
}

double cos_large(double x) {
    return trig_large_arg_handler(x, true);
}
